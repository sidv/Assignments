...
def add_employee():
    ...
    payload = {"id": id, "name": name, "email": email,
               "username": username, "salary": salary, "pre_comp": pre_comp}
...
def search_emp_by_name():
    ...
    payload = {"name": name}
    header = {"Content-type": "application/json"}
    res = req.post(SEARCH_BY_NAME_URL,
...
def update_employee_username():
    ...
    header = {"Content-type": "application/json"}
    res = req.post(UPDATE_EMP_USERNAME_URL,
                   data=json.dumps(payload), headers=header)
    print(res.text)
...
def update_employee_email():
    ...
    header = {"Content-type": "application/json"}
    res = req.post(UPDATE_EMP_EMAIL_URL,
                   data=json.dumps(payload), headers=header)
    print(res.text)
